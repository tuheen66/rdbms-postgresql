...
-- ! the query that returns only one column/row is called scaler query
...
--! Sub query used in SELECT must return a single value
SELECT *, (SELECT sum(salary) FROM employees) FROM employees

SELECT * FROM (SELECT department_name, sum(salary) sum_dept_salary FROM employees GROUP BY department_name)
