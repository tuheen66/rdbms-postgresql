...
SELECT department_name, count(employee_id) as No_of_employees FROM employees
JOIN departments USING(department_id)
GROUP BY department_name
...
--? 6.Find customers who place more than 2 orders and calculate the total amount spent by each of these customers

SELECT customer_id, count(order_id) as no_of_orders, sum(total_amount) as total_spent FROM orders
